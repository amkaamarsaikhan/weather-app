...
function showLoading() {
    ...
    removeLoader();
    resultSection.insertAdjacentHTML('afterbegin', '<div id="loader" class="loading" style="text-align: center; padding: 20px;">Loading...</div>');
}
...
async function fetchWeatherData(cityName) {
    try {
        ...
        if (!weatherRes.ok) throw new Error("Weather server not responding.");
...
function renderWeatherCard(name, country, data) {
    const info = weatherInfo[data.weather_code] || { desc: 'Unknown', icon: '🌈' };
    ...
    const cardHtml = `
        <div class="weather-card" style="background: var(--glass-bg); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid var(--glass-border); border-radius: 24px; padding: 25px; box-shadow: 0 15px 35px rgba(0, 0, 0, 0.2); animation: slideUp 0.6s ease; margin-bottom: 20px;">
            ...
            <div class="weather-details" style="margin-top: 15px; border-top: 1px solid var(--glass-border); padding-top: 15px;">
                <div class="detail-item"><span>Humidity</span><br><strong>${data.relative_humidity_2m}%</strong></div>
                <div class="detail-item"><span>Wind</span><br><strong>${data.wind_speed_10m} m/s</strong></div>
                <div class="detail-item"><span>Pressure</span><br><strong>${Math.round(data.surface_pressure)} hPa</strong></div>
                <div class="detail-item"><span>Visibility</span><br><strong>${(data.visibility / 1000).toFixed(1)} km</strong></div>
            </div>
        </div>
    `;
